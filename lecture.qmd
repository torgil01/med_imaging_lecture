...
We have now loaded the data from the two images. The image headers are in `t1_nii` and `label_nii`, while the image data is in `t1` and `label`.
...
Abowe is the complete header from the T1 image. You can get a specific value using `t1_nii.header["key"]`. For example, the voxel size is `t1_nii.header["pixdim"][1]` and the data type is `t1_nii.header["datatype"]`

## Viewing the image
Detailed inspection of the images are best done using a dedicated image viewer, but we can also use `matplotlib` to view the images.
